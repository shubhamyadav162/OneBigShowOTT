...
const SubscriptionScreen = () => {
    ...
    const loadSubscriptionData = async () => {
      ...
      try {
        ...
        if (plansRes && plansRes.data && plansRes.success && Array.isArray(plansRes.data.plans) && plansRes.data.plans.length > 0) {
          ...
          finalPlansToSet = getEnhancedDefaultPlans();
        }
      } catch (apiPlanError) {
      ...
      try {
        if (user && user.id) {
          ...
          if (currentRes && currentRes.data && currentRes.success) {
            ...
            const planObj = finalPlansToSet.find(p => p.id === planId) || null;
            setCurrentPlan({
              planId,
  ...
  const handleSubscribe = async (plan) => {
    if (!user || !user.id) {
      ...
      Alert.alert('त्रुटि', 'उपयोगकर्ता सत्र नहीं मिला। कृपया पुनः लॉगिन करें और पुनः प्रयास करें।');
      return;
    }

    if (!plan || !plan.id) {
      console.error('Invalid plan data on subscribe:', plan);
      Alert.alert('त्रुटि', 'अमान्य योजना डेटा। कृपया बाद में पुनः प्रयास करें।');
      return;
    }
    ...
    try {
      ...
      if (functionError) {
        ...
        throw new Error(functionError.message || 'भुगतान प्रारंभ करने में त्रुटि हुई');
      }
      
      if (!data) {
        throw new Error('भुगतान प्रारंभ करने से कोई डेटा नहीं मिला');
      }
      ...
      if (data.success && data.paymentLink) {
        ...
        throw new Error(data?.error || 'पेमेंट शुरू करने में विफल रहा। सर्वर से कोई भुगतान URL नहीं मिला।');
      }
    } catch (error) {
      console.error('Error initiating payment:', error.message, error.details || error);
      let displayErrorMessage = 'भुगतान शुरू करने में त्रुटि हुई है। कृपया बाद में पुनः प्रयास करें।';
      if (error.message && error.message.toLowerCase().includes('network request failed')) {
        displayErrorMessage = 'नेटवर्क त्रुटि। कृपया अपनी इंटरनेट कनेक्टिविटी जांचें और पुनः प्रयास करें।';
      } else if (error.message && error.message.includes('user')) {
        displayErrorMessage = 'उपयोगकर्ता सत्र में समस्या। कृपया पुनः लॉगिन करें और पुनः प्रयास करें।';
      }
      Alert.alert('त्रुटि', displayErrorMessage);
    } finally {
  ...
  const renderPlanItem = ({ item }) => (
    item ? (
      <View style={[
        ...
        <Text style={styles.planDuration}>{item.duration_days || item.duration || '30'} दिन</Text>
        <Text style={styles.planDescription}>{item.description || `${item.duration_days || 'Standard'} दिनों की योजना`}</Text>
        ...
        >
          <Text style={styles.continueButtonText}>
            {(isProcessing && selectedPlan === item.id) ? 'प्रोसेसिंग...' : (currentPlan?.planId === item.id && currentPlan?.status === 'active' ? 'Current Plan' : 'सब्सक्राइब करें')}
          </Text>
        </TouchableOpacity>
      </View>
    ) : null
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>सदस्यता योजनाएँ</Text>
      ...
      <ScrollView style={styles.container}>
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color={theme.colors.primary} />
            <Text style={styles.loadingText}>सदस्यता योजनाएँ लोड हो रही हैं...</Text>
          </View>
        ) : (
          <>
            {/* Show current subscription status if available */}
            {currentPlan || isSubscribed ? (
              <View style={styles.currentPlanCard}>
                <Text style={styles.currentPlanTitle}>वर्तमान सदस्यता</Text>
                <View style={styles.currentPlanContent}>
                  <View style={styles.currentPlanRow}>
                    <Text style={styles.currentPlanLabel}>योजना:</Text>
                    <Text style={styles.currentPlanValue}>
                      {subscriptionDetails?.planName || currentPlan?.name || 'अज्ञात योजना'}
                    </Text>
                  </View>
                  <View style={styles.currentPlanRow}>
                    <Text style={styles.currentPlanLabel}>स्थिति:</Text>
                    <View style={[
                      styles.statusBadge, 
                      { backgroundColor: isSubscribed ? theme.colors.success : theme.colors.error }
                    ]}>
                      <Text style={styles.statusBadgeText}>
                        {isSubscribed ? 'सक्रिय' : 'समाप्त'}
                      </Text>
                    </View>
                  </View>
                  <View style={styles.currentPlanRow}>
                    <Text style={styles.currentPlanLabel}>समाप्ति:</Text>
                    <Text style={styles.currentPlanValue}>
                ...
                >
                  <Text style={styles.manageSubscriptionText}>सदस्यता प्रबंधित करें</Text>
                </TouchableOpacity>
              </View>
            ) : null}
            ...
            <Text style={styles.sectionTitle}>
              {currentPlan || isSubscribed ? 'नई सदस्यता योजनाएँ' : 'उपलब्ध सदस्यता योजनाएँ'}
            </Text>
            ...
            {plans.length > 0 ? (
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>कोई सदस्यता योजना उपलब्ध नहीं है</Text>
              </View>
            )}
            ...
            <View style={styles.infoContainer}>
              ...
              <Text style={styles.infoText}>
                • सदस्यता लेकर आप हमारे सभी प्रीमियम कंटेंट का आनंद ले सकते हैं।
              </Text>
              <Text style={styles.infoText}>
                • सदस्यता शुल्क वापस नहीं किया जाएगा।
              </Text>
              <Text style={styles.infoText}>
                • विभिन्न योजनाओं के लिए अलग-अलग मूल्य और अवधि हैं।
              </Text>
              <Text style={styles.infoText}>
                • किसी भी समस्या के लिए कृपया हमारे ग्राहक सेवा से संपर्क करें।
              </Text>
            </View>
          </>
        )}
